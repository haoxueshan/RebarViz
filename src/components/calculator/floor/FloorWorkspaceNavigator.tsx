...
/**
 * UI V3.1（PRD 20-22）：Rail 按钮真正按分类定位 Navigator。
 * Overlay/Drawer 通过 activeSection 只显示对应 Section；null 表示全量（Desktop 展开态）。
 */
export type FloorNavigatorSection = "slabs" | "openings" | "roles" | "through" | "add";

export const FLOOR_NAVIGATOR_SECTION_LABELS: Record<FloorNavigatorSection, string> = {
  slabs: "楼层对象",
  openings: "洞口导航",
  roles: "主副筋导航",
  through: "通墙导航",
...
export function FloorWorkspaceNavigator({
  ...
  compact?: boolean;
  /** UI V3.1：Overlay 只显示该 Section；null 时显示全部（Desktop 展开态）。 */
  activeSection?: FloorNavigatorSection | null;
  /** UI V3（PRD 59-64）：Rail 按钮打开对应分类的 Navigator Overlay。 */
  onOpenOverlay?: (section: FloorNavigatorSection) => void;
  ...
  if (compact) {
    // UI V3.1（PRD 20-22）：Rail 按钮按分类打开对应 Section 的 Overlay。
    // 「新增」不再无提示直接创建板区，而是打开 Overlay 定位到新增操作。
    const railButton = (label: string, icon: React.ReactNode, section: FloorNavigatorSection, active = false) => (
      <button key={label} type="button" title={label} aria-label={`导航-${label}`} aria-current={active ? "true" : undefined} onClick={() => onOpenOverlay?.(section)} className={`flex size-11 items-center justify-center rounded-xl ${active ? "bg-blue-600 text-white" : "text-slate-600 hover:bg-slate-100"}`}>{icon}</button>
    );
    return (
      <nav className="flex h-full flex-col items-center gap-2 border-r border-slate-200 bg-white p-1.5" aria-label="工作台对象快捷导航">
        {stage === "plan" ? (
          <>
            {railButton("对象", <Grid2X2 size={17} />, "slabs", selection?.kind === "slab")}
            {railButton("洞口", <DoorOpen size={17} />, "openings", selection?.kind === "opening")}
            {railButton("新增", <Plus size={17} />, "add")}
          </>
        ) : (
          <>
            {railButton("板区", <Grid2X2 size={17} />, "slabs", selection?.kind === "slab")}
            {railButton("主副筋", <Circle size={17} />, "roles")}
            {stage === "top" && railButton("通墙", <Link2 size={17} />, "through", Boolean(selectedThroughPathId))}
          </>
        )}
      </nav>
    );
  ...
  const sectionFiltered = activeSection !== null;
  const showSection = (section: FloorNavigatorSection) => !sectionFiltered || activeSection === section;
  const showSlabs = showSection("slabs");
  const showOpenings = showSection("openings");
  const showRoles = showSection("roles");
  const showThrough = showSection("through");
  const showAdd = showSection("add");
  const headerTitle = stage === "plan" ? "楼层对象" : stage === "bottom" ? "地筋设置" : "面筋设置";
    ...
    <nav className="flex h-full min-h-0 flex-col bg-white" aria-label="整层工作台导航" data-testid="floor-workspace-navigator">
      <div className="border-b border-slate-200 p-3">
        <div className="flex items-start justify-between gap-2">
          <div><h2 className="text-sm font-bold text-slate-950">{headerTitle}</h2><p className="mt-0.5 text-[11px] text-slate-500">{plan.slabs.length}板区 · {plan.openings.length}洞口{localCount > 0 ? ` · ${localCount}局部` : ""}{stage === "top" && throughItems.length > 0 ? ` · ${throughItems.length}通墙` : ""}</p></div>
        </div>
        {stage === "plan" && !sectionFiltered && <div className="mt-3 grid grid-cols-2 gap-2"><button type="button" aria-label="添加板区" onClick={onAddSlab} className="inline-flex min-h-11 items-center justify-center gap-1 rounded-xl bg-blue-600 px-2 text-xs font-semibold text-white"><Plus size={15} />板区</button><button type="button" aria-label="添加洞口" onClick={onAddOpening} className="inline-flex min-h-11 items-center justify-center gap-1 rounded-xl bg-rose-600 px-2 text-xs font-semibold text-white"><DoorOpen size={15} />洞口</button></div>}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-4">
        {showAdd && stage === "plan" && (
          <div data-navigator-section="add" className="space-y-2 pt-2">
            <p className="px-2 text-xs leading-5 text-slate-500">选择要新增的对象类型：</p>
            <button type="button" aria-label="新增板区" onClick={onAddSlab} className="flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-blue-600 px-2 text-sm font-semibold text-white"><Plus size={15} />新增板区</button>
            <button type="button" aria-label="新增洞口" onClick={onAddOpening} className="flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-rose-600 px-2 text-sm font-semibold text-white"><DoorOpen size={15} />新增洞口</button>
          </div>
        )}
        {showSlabs && (
